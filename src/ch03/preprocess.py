...
resize and nothing more. SigLIP's own ``[-1, 1]`` normalization lives
inside ``VisionEncoder`` so the boundary here stays the ``[0, 1]``
convention Chapter 2 established.
"""
...
def preprocess_image(image: torch.Tensor) -> torch.Tensor:
    ...
    with the spatial dimensions resized to ``224 x 224``. Pixel values
    stay in ``[0, 1]``; SigLIP normalization is applied later, inside the
    vision encoder.
    """
    single = image.ndim == 3
    if single:
