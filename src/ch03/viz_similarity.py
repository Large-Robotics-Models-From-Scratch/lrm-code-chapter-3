...
Pick one patch (say, on the cube), take its frozen SigLIP feature vector,
and measure cosine similarity to every other patch. Reshape to the 14x14
grid and the patches the encoder represents like the query light up: the
rest of the cube for a cube query, the arm for an arm query. This reads
the frozen features directly, so it shows what the encoder represents
with no training and no attention machinery.
