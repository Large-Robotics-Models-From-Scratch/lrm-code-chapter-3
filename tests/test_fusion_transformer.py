...
def test_output_shape_matches_input():
    fusion = FusionTransformer(hidden_dim=512, num_layers=6, num_heads=8)
    tokens = torch.rand(2, 50, 512)
    out = fusion(tokens)
...
def test_output_attentions_shape():
    ...
    assert attentions[0].shape == (2, 8, 50, 50)
...
def test_causal_mask_blocks_future_influence():
    ...
    tokens = torch.rand(1, 6, 512)
    perturbed = tokens.clone()
    perturbed[:, -1] = torch.rand(512)  # only the last (future) token
    with torch.no_grad():
        base = fusion(tokens)
