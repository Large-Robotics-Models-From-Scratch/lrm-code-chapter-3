...
def test_ch4_contract_shape(backbone, dummy_state):
    ...
    input_ids = torch.tensor(
        [backbone.build_input_ids(text_ids)] * 2
    )
...
def test_image_block_is_392(backbone):
    ...
    row = backbone.build_input_ids([5, 6, 7])
...
def test_template_order_is_image_text_state(backbone):
    ...
    text_ids = [5, 6, 7]
    row = backbone.build_input_ids(text_ids)
...
def test_no_vocab_expansion(backbone):
    ...
    assert (
        backbone.language_backbone.config.vocab_size == SMOLLM_VOCAB
    )
